...
export default function Page() {
  ...
  useEffect(() => {
    async function init() {
      try {
        ...
        const response = await fetch("/api/projects")
        const dbProjects: Project[] = response.ok ? await response.json() : []

        if (dbProjects && dbProjects.length > 0) {
          setProjects(dbProjects)
          const savedActiveId = localStorage.getItem("nodepad-active-project")
          setActiveProjectId(savedActiveId && dbProjects.some(p => p.id === savedActiveId) 
            ? savedActiveId 
            : dbProjects[0].id)
          setIsLoaded(true)
          return
        ...
        // If DB is empty, fall back to localStorage and migrate
        ...
        // Trigger migration to DB
        console.info("Migrating localStorage data to Supabase...")
  ...
  // 2. Persistence: Local Storage Mirror (for session continuity)
  useEffect(() => {
    if (!isLoaded) return
    localStorage.setItem("nodepad-active-project", activeProjectId)
  }, [activeProjectId, isLoaded])

  // 3. Real-time DB Sync (Debounced per project)
  const lastSyncRef = useRef<Record<string, string>>({})
  useEffect(() => {
    if (!isLoaded || projects.length === 0) return
    
    // Sync the current active project to DB if it changed
    const currentProject = projects.find(p => p.id === activeProjectId)
    if (!currentProject) return

    const projectData = JSON.stringify(currentProject)
    if (lastSyncRef.current[activeProjectId] === projectData) return

    const timer = setTimeout(() => {
      saveProjectToDb(currentProject)
      lastSyncRef.current[activeProjectId] = projectData
    }, 2000)

    return () => clearTimeout(timer)
  }, [projects, activeProjectId, isLoaded, saveProjectToDb])

  // A ref to read current projects without causing re-renders or stale closures
  const projectsRef = useRef(projects)
  useEffect(() => { projectsRef.current = projects }, [projects])

  // Stable ref to active blocks
  ...
  const handleImportFile = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (event) => {
      ...
      try {
        const imported = parseNodepadFile(content, projects.map(p => p.name))
        setProjects(prev => [...prev, imported])
        setActiveProjectId(imported.id)
        showUndoToast(`Imported "${imported.name}"`)
      } catch (err) {
        console.error("Import failed:", err)
        showUndoToast("Import failed")
      }
    }
    reader.readAsText(file)
    e.target.value = ""
  }, [projects, showUndoToast])
